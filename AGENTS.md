...
This project researches and implements a reliable, long-term, and evolvable memory system for inspection robots. It should continuously accumulate, retrieve, and use experience to improve environment understanding, anomaly detection, planning, and response, ultimately supporting safe, verifiable, and reversible self-evolution.

Research spans working memory, episodic memory, semantic and schema memory, memory-driven prediction and planning, and metamemory. The current campus demo prioritizes the first three levels. Do not lock in a model, dataset, technical approach, robot platform, or submission target without an explicit request.

Preserve the research process from literature review, problem formulation, and hypotheses through experiments, analysis, and paper writing. Existing surveys and proposals are exploratory inputs, not final approaches or validated conclusions. See [Project conventions](docs/project-guide.md) for project-specific structure, evidence, and reproducibility requirements.
